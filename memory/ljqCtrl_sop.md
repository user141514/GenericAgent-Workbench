...
ctrl = EasyCtrl("窗口标题")  # 自动处理窗口激活+DPI转换

ctrl.click(100, 200)           # 点击客户区坐标(自动DPI转换)
ctrl.double_click(100, 200)    # 双击
ctrl.type_text("Hello")        # 输入文本(通过剪贴板)
ctrl.find_and_click("btn.png") # 找图并点击
ctrl.press("ctrl+c")           # 快捷键
ctrl.move_to(100, 200)         # 移动鼠标

# 便捷函数(无需实例化)
click_window("窗口标题", x, y)
type_text("文本")
find_and_click("image.png", title="窗口标题")
```
